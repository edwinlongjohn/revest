            ...
            <div class="row justify-content-center text-center ">
                <div class="col-xl-6 col-lg-6 col-md-8">
                    <div class="login-form">
                        ...
                        <form id="contactForm" method="POST" action="{{ route('login') }}"class="log-form">@csrf
                            @foreach ($errors->all() as $error)
                                <p class="text-danger text-center">{{ $error }}</p>
                            @endforeach

                            <input type="text" value="{{ old('email') }}" name="email" id="name"
                                class="form-control" placeholder="Username" required
                                data-error="Please enter your name">
                            <input type="password" name="password" id="msg_subject" class="form-control"
                            ...
                            <div class="sign-icon">
                                ...
                                <div class="acc-not">{{ __("Don't have an account?") }} <a
                                        href="{{ route('register') }}">{{ __('Signup') }}</a></div>
                            </div>
                        </form>
                    </div>
                </div>
